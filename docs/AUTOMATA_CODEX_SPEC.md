...
- **ユーザー別設定管理**: GitLabユーザー名ベースでのAPIキー管理と設定分離
...
**主要機能**:
- **ユーザー登録・管理**: GitLabユーザー名ベースのユーザー管理
...
4. WorkflowBuilderを使用してExecutorを追加（TaskContextInit、PlanEnvSetup等）
...
**主要メソッド**:
- `create_task_context_init()`: TaskContextInitExecutor生成
...
2. TaskContextInitExecutor生成時は外部依存不要でインスタンス化
...
**責務**: GitLabユーザー名からユーザー設定を取得し、ワークフロー内で利用可能にする
...
**処理フロー**:
1. ワークフローからGitLabユーザー名を受け取る
2. User Config APIに問い合わせる (GET /api/v1/config/{username})
...
graph TB
    ...
    subgraph "Executor層"
        ...
        TaskContextInitExec[TaskContextInitExecutor<br/>タスクコンテキスト初期化]
    ...
    ExecutorFactory -->|Executor生成| TaskContextInitExec
    ...
    %% Workflow実行
    Workflow -->|Executor実行| TaskContextInitExec
...
- **ExecutorFactory**: カスタムExecutorを生成（TaskContextInit、ContentTransfer、EnvironmentSetup、BranchMerge）
...
| `username` | str | タスク実行ユーザーのGitLabユーザー名 | TaskContextInitExecutor実行時 |
...
PostgreSqlChatHistoryProvider.save_messages()メソッドがメッセージを保存した後、自動的にContextCompressionService.check_and_compress_async()を呼び出す。この際、usernameを渡すことで、ContextCompressionServiceがuser_configsテーブルからユーザーの圧縮設定を読み込む。
...
5. **データベース記録**:
   ...
   - 記録フィールド:
     - username: ワークフローコンテキストから取得（token_usageテーブルのFK: users.username）
...
6. **メトリクス送信**:
   ...
   - ラベル: model、node_id、username
   - 値: total_tokens

**並行実行時の動作**:
- 複数タスクが同時に実行される場合でも、各タスクのusernameとtask_uuidで区別

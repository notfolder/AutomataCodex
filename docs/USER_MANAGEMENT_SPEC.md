...
sequenceDiagram
    ...
    Admin->>Web: ユーザー登録 (email, username, password, role)
    Web->>API: POST /api/v1/users
    API->>Hash: パスワードハッシュ化 (bcrypt, cost 12)
    Hash-->>API: password_hash
    API->>Enc: APIキー暗号化
    Enc-->>API: 暗号化済みキー
    API->>DB: INSERT users (email, username, password_hash, role)
    API->>DB: INSERT user_configs (user_email, api_key_encrypted, ...)
    DB-->>API: 成功
    API-->>Web: 登録完了
    Web-->>Admin: 確認画面
```
...
| user | 一般ユーザー | 自分のLLM設定とコンテキスト圧縮設定の変更、自分のパスワード変更のみ可能。Web管理画面へのアクセス不可 |
...
sequenceDiagram
    ...
    CLI->>Input: メールアドレス、ユーザー名、パスワードを要求
    Input-->>CLI: 入力値
    CLI->>CLI: バリデーション（メール形式、パスワード要件）
    CLI->>DB: 既存ユーザー確認 (SELECT FROM users WHERE email = ?)
    alt ユーザーが既に存在
        DB-->>CLI: ユーザー存在
        CLI->>CLI: エラー: 既に登録済み
    else ユーザーが存在しない
        ...
        CLI->>DB: INSERT INTO users (email, username, password_hash, role, is_active)
        CLI->>DB: INSERT INTO user_configs (user_email, デフォルト設定)
        CLI->>DB: COMMIT
        DB-->>CLI: 成功
        CLI->>CLI: 成功メッセージ表示
    end
...
- **パスワード**: 
  ...
  - 数字を含む
  - 記号を含む
- **既存ユーザーチェック**: 同じメールアドレスのユーザーが存在しないこと
...
- **既存ユーザーエラー**: メールアドレスが既に登録されている場合、エラーメッセージを表示して終了
...
- Body: ユーザー情報（email、username、password、role、is_active）とLLM設定（圧縮設定は任意、未指定の場合はデフォルト値が適用される）
- Validation: 
...
- Purpose: ユーザー一覧取得
- Authentication: Bearer Token (Admin)
- Response: ユーザーリスト（email、username、role、is_active、created_at）
...
| SC-14 | パスワード変更画面 | `/settings/password` | ユーザー自身のパスワード変更。PUT /api/v1/users/{email}/passwordで変更 |
...
│  ← ユーザー編集画面へ戻る                               │
│                                                         │
│  パスワード変更: user1@example.com                      │
│                                                         │
│  ┌──────────────────────────────────────────────┐      │
│  │ 管理者権限でパスワード変更                   │      │
│  │                                              │      │
│  │ 対象ユーザー: user1@example.com              │      │
│  │                                              │      │
│  │ 新しいパスワード *                           │      │
│  │ [______________________________]             │      │
│  │ (8文字以上、英数字と記号を含む)             │      │

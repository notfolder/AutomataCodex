...
| SC-07 | プロンプト管理 | `/users/:id/prompts` | エージェント別プロンプト上書き管理 |
| SC-08 | ワークフロー定義一覧 | `/workflows` | システムプリセット・ユーザー作成ワークフロー一覧 |
| SC-09 | ワークフロー定義詳細 | `/workflows/:id` | グラフ定義・エージェント定義・プロンプト定義の閲覧 |
| SC-10 | ワークフロー定義作成 | `/workflows/new` | 新規ワークフロー定義作成 |
| SC-11 | ワークフロー定義編集 | `/workflows/:id/edit` | ワークフロー定義編集（ユーザー作成のみ） |
| SC-12 | トークン使用量統計 | `/statistics/tokens` | ユーザー別トークン消費統計・グラフ表示 |
| SC-13 | タスク実行履歴 | `/tasks` | タスク実行履歴一覧・ステータス確認 |
...
flowchart TD
    ...
    Dashboard -->|ワークフロー管理| WorkflowList[SC-08: ワークフロー定義一覧]
    Dashboard -->|統計情報| TokenStats[SC-12: トークン使用量統計]
    Dashboard -->|タスク履歴| TaskHistory[SC-13: タスク実行履歴]
    ...
    UserDetail -->|プロンプト管理| PromptManage[SC-07: プロンプト管理]
    UserDetail -->|一覧へ戻る| UserList
    
    UserEdit -->|保存| UserDetail
    UserEdit -->|キャンセル| UserDetail
    
    PromptManage -->|保存| UserDetail
    PromptManage -->|キャンセル| UserDetail
    
    WorkflowList -->|新規作成| WorkflowNew[SC-10: ワークフロー定義作成]
    WorkflowList -->|詳細表示| WorkflowDetail[SC-09: ワークフロー定義詳細]
    
    WorkflowNew -->|作成完了| WorkflowList
    
    WorkflowDetail -->|編集| WorkflowEdit[SC-11: ワークフロー定義編集]
...
│  ← ユーザー詳細へ                                       │
│                                                         │
│  プロンプト管理: user1@example.com                      │
│                                                         │
│  各エージェントのプロンプトをカスタマイズできます。     │

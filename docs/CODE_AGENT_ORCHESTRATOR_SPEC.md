...
- `environment_id`: Docker環境ID（requires_environment=trueの場合）
...
- `get_environment_id()`: ExecutionEnvironmentManagerから`node_id`を使用して環境IDを取得する
...
- **execution**: プラン取得→環境ID取得（`get_environment_id()`で`node_id`を使用）→進捗報告（開始）→LLM呼び出し（実装/生成）→進捗報告（LLM応答）→ファイル操作（MCPツール）→git操作→進捗報告（実行完了）→`execution_environments[agent_definition_id] = env_id`として保存→コンテキスト保存
- **review**: MR差分取得→`execution_environments`辞書から環境ID取得→進捗報告（開始）→LLM呼び出し（レビュー）→進捗報告（LLM応答）→コメント生成→進捗報告（レビュー完了）→コンテキスト保存
...
**責務**: グラフ定義で`requires_environment: true`のノード分のDocker環境を準備する

**処理フロー**:
1. `validate_graph_definition()`が返した`requires_environment: true`のノード数を取得
2. ExecutionEnvironmentManagerを使用して必要数のDocker環境を起動
3. 環境IDをワークフローコンテキストに保存
4. 各実行エージェントが環境IDを使用してコード実行・テスト実行を行う
...
`ExecutionEnvironmentManager`はDocker環境のライフサイクル（作成、割り当て、クリーンアップ）を管理する独自実装クラスである。グラフ定義の`requires_environment: true`ノード数に基づいて環境プールを準備し、各ノードに一意の環境を割り当てる。プロジェクト言語に応じた適切なDockerイメージを選択して環境を作成する。

#### 8.8.2 責務

1. **環境名マッピング管理**: 環境名（python、node等）からDockerイメージ名へのマッピングを管理
2. **環境プール管理**: ワークフロー開始時に必要な数のDocker環境を選択されたイメージで一括作成
3. **環境割り当て**: 各`requires_environment: true`ノードに一意の環境IDを割り当て
...
| メソッド | 説明 |
|---------|------|
| `prepare_environments(count, environment_name)` | 指定された環境名のイメージでcount個のDocker環境を一括作成し、環境IDリストを返す |
...
2. 指定された数だけループ処理を行う
3. 各反復で一意の環境ID（UUID形式）を生成する
4. Docker APIを呼び出して選択されたイメージでDocker環境を作成する（並列化可能）

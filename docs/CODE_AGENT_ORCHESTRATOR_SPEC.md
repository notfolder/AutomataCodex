...
```mermaid
flowchart TD
    A[ユーザーがプリセット一覧を確認<br/>GET /api/v1/workflow_definitions] --> B[ベースにするプリセットを取得<br/>GET /api/v1/workflow_definitions/{id}]
    B --> C[JSONを編集してカスタム定義を作成<br/>POST /api/v1/workflow_definitions<br/>is_preset=false, created_by=user_id]
    C --> D[カスタム定義をワークフローとして選択<br/>PUT /api/v1/users/{user_id}/workflow_setting]
    D --> E[以降のタスクでカスタム定義が使用される]
    C --> F[カスタム定義を更新<br/>PUT /api/v1/workflow_definitions/{id}]
    C --> G[カスタム定義を削除<br/>DELETE /api/v1/workflow_definitions/{id}]
```

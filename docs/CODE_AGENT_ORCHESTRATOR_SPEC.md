...
- `error_handler` - Error Handler Agent
...
- **ユーザー管理**: ユーザーCRUD操作
- **設定管理**: LLM設定の編集
- **プロンプト管理**: エージェントごとのプロンプト上書き編集（全14エージェント対応）
...
| Code Generation Planning Agent | コード生成タスクの実行計画生成 | タスク内容、仕様書 | コード生成アクションプラン |
| Bug Fix Planning Agent | バグ修正タスクの実行計画生成 | タスク内容、エラー情報 | バグ修正アクションプラン |
| Test Creation Planning Agent | テスト作成タスクの実行計画生成 | タスク内容、対象コード | テスト作成アクションプラン |
| Documentation Planning Agent | ドキュメント生成タスクの実行計画生成 | タスク内容、コードベース | ドキュメント生成アクションプラン |
...
**プロンプト管理**:
- 各エージェントのデフォルトプロンプト詳細はPROMPTS.mdを参照
- ユーザーごとにプロンプトを上書き可能（`agent_prompt_overrides`テーブルで管理）
- プロンプト取得優先順位:
  1. `agent_prompt_overrides`テーブルのユーザー固有プロンプト
  2. PROMPTS.mdのデフォルトプロンプト
- LLM呼び出し時には、プロンプト冒頭にAGENTS.mdの内容を含める

**プロンプト上書き実装**:
- `AgentFactory`が`AIAgent`生成時に`agent_prompt_overrides`テーブルを確認
- ユーザーのプロンプト上書きが存在する場合、デフォルトプロンプトの代わりに使用
- `ChatClientAgentOptions.instructions`にカスタムプロンプトを設定

### 4.2 Factory設計

#### 4.2.1 WorkflowFactory

**責務**: タスクタイプに応じて適切なWorkflowを生成する
...
- `PromptOverrideManager`: プロンプト管理

**主要メソッド**:
- `create_code_generation_workflow(user_id)`: コード生成ワークフロー生成
- `create_bug_fix_workflow(user_id)`: バグ修正ワークフロー生成
- `create_test_generation_workflow(user_id)`: テスト生成ワークフロー生成
- `create_refactoring_workflow(user_id)`: リファクタリングワークフロー生成
- `create_document_workflow(user_id)`: ドキュメント生成ワークフロー生成
- `create_code_review_workflow(user_id)`: コードレビューワークフロー生成

**実装方針**:
1. コンストラクタで各Factory、Manager、Middlewareを保持
2. ワークフロー生成時に新しいsession_idを生成
3. WorkflowBuilderを使用してExecutorを追加（UserResolver、EnvironmentSetup等）
4. WorkflowBuilderを使用してAIAgentを追加（Planning Agent、Execution Agent、Review Agent）
5. TokenUsageMiddlewareをWorkflowBuilderに追加
6. WorkflowBuilderのbuild()メソッドでWorkflowオブジェクトを生成して返却
7. 他のワークフロー種別（バグ修正、テスト生成、リファクタリング、ドキュメント作成）も同様のパターンで実装
...
Agent Frameworkの[`ChatCompletionAgent`](https://learn.microsoft.com/en-us/semantic-kernel/frameworks/agent/agent-chat?pivots=programming-language-python)を基底として、以下の3つの抽象基底クラスを定義し、各具体的エージェントはこれらを継承する。

```
ChatCompletionAgent (Agent Framework標準)
├─ BasePlanningAgent (抽象基底クラス)
│  ├─ CodeGenerationPlanningAgent
│  ├─ BugFixPlanningAgent
│  ├─ TestCreationPlanningAgent
│  └─ DocumentationPlanningAgent
├─ BaseReflectionAgent (抽象基底クラス)
│  └─ PlanReflectionAgent
├─ BaseExecutionAgent (抽象基底クラス)
│  ├─ CodeGenerationAgent
│  ├─ BugFixAgent
│  ├─ DocumentationAgent
│  └─ TestCreationAgent
└─ BaseReviewAgent (抽象基底クラス)
   ├─ TestExecutionEvaluationAgent
   ├─ CodeReviewAgent
...
**責務**: すべてのPlanning Agentsの共通機能を提供する

**抽象メソッド**:
- `plan_async()`: プランニングを実行（各具体的エージェントで実装）

**共通機能**:
1. **コンテキスト取得**: ワークフローコンテキストから要件情報、プロジェクト情報を取得
2. **仕様書確認**: docs/SPEC_*.md等の仕様ファイルの存在確認
3. **プラン生成**: LLMにプロンプトを送信し、実行計画を生成
4. **Todoリスト作成**: `create_todo_list`ツールで永続化
5. **GitLab投稿**: `sync_to_gitlab`ツールでTodoリストを投稿
6. **コンテキスト保存**: ワークフローコンテキストにプランを保存

**共通ヘルパーメソッド**:
- `get_chat_history()`: 会話履歴を取得
- `get_context()`: コンテキストを取得
- `store_result(result)`: 結果を保存
- `create_action_plan()`: アクションプランを生成
- `verify_spec_exists()`: 仕様書の存在確認

**共通ツール**（`ChatClientAgentOptions.tools`に登録）:
- コードベース検索ツール（Agent FrameworkのMCPツールとして統合されたtext-editor MCPサーバーのツール）
- ファイル読み込みツール（Agent FrameworkのMCPツールとして統合されたtext-editor MCPサーバーのツール）
- `create_todo_list`ツール（Agent Frameworkのネイティブツール）
- `sync_to_gitlab`ツール（Agent Frameworkのネイティブツール）

**実装方法**:
- [ChatCompletionAgent](https://learn.microsoft.com/en-us/semantic-kernel/frameworks/agent/agent-chat?pivots=programming-language-python)をラップする
- 共通処理を抽象メソッドとして定義し、各具体的エージェントで実装する

#### BaseReflectionAgent（抽象基底クラス）

**継承元**: [`ChatCompletionAgent`](https://learn.microsoft.com/en-us/semantic-kernel/frameworks/agent/agent-chat?pivots=programming-language-python)

**責務**: プランニング後にプランを検証・改善するための共通機能を提供する

**抽象メソッド**:
- `reflect_async()`: プラン検証を実行（各具体的エージェントで実装）

**共通機能**:
1. **プラン取得**: ワークフローコンテキストから実行計画とTodoリストを取得
2. **プラン検証**: LLMにプランの妥当性を検証依頼
3. **問題点特定**: プランの矛盾、不明確な点、不足している情報を特定
4. **改善提案生成**: 具体的な改善案を生成
5. **改善判定**: 改善が必要かどうかを判定
6. **GitLab投稿**: 検証結果と改善提案を投稿
7. **コンテキスト保存**: 検証結果をワークフローコンテキストに保存
...
- `store_result(result)`: 結果を保存
- `get_plan()`: 実行計画を取得
- `identify_issues(plan)`: プランの問題点を特定
- `generate_improvements(issues)`: 改善案を生成

**共通ツール**（`ChatClientAgentOptions.tools`に登録）:
- コードベース検索ツール（Agent FrameworkのMCPツールとして統合されたtext-editor MCPサーバーのツール）
- ファイル読み込みツール（Agent FrameworkのMCPツールとして統合されたtext-editor MCPサーバーのツール）
- `get_todo_list`ツール（Agent Frameworkのネイティブツール）
- `sync_to_gitlab`ツール（Agent Frameworkのネイティブツール）
...
**責務**: すべてのExecution Agentsの共通機能を提供する

**抽象メソッド**:
- `execute_async()`: 実行を行う（各具体的エージェントで実装）

**共通機能**:
1. **プラン取得**: ワークフローコンテキストから実行計画を取得
2. **コード/ドキュメント生成**: LLMに生成依頼
3. **ファイル操作**: MCP Server (text-editor)で編集
4. **実行結果記録**: ワークフローコンテキストで結果を保存
5. **git操作**: ExecutionEnvironmentManagerを使用してコミット

**共通ヘルパーメソッド**:
- `get_chat_history()`: 会話履歴を取得
- `get_context()`: コンテキストを取得
- `store_result(result)`: 結果を保存
- `invoke_mcp_tool(tool_name, params)`: MCPツールを呼び出す

**共通ツール**（`ChatClientAgentOptions.tools`に登録）:
- text-editor MCPツール（Agent FrameworkのMCPツールとして統合）
- command-executor MCPツール（Agent FrameworkのMCPツールとして統合）
- ExecutionEnvironmentManager（Docker環境操作、Agent Frameworkのネイティブツール）
...
**責務**: すべてのReview Agentsの共通機能を提供する

**抽象メソッド**:
- `review_async()`: レビューを実行（各具体的エージェントで実装）

**共通機能**:
1. **差分取得**: GitLab APIでMR差分を取得
2. **レビュー実行**: LLMにレビュー依頼
3. **コメント生成**: 具体的な改善提案を生成
4. **GitLab投稿**: GitLab API経由でMRコメントを投稿
5. **結果記録**: ワークフローコンテキストで結果を保存
...
- `store_result(result)`: 結果を保存
- `get_mr_diff()`: MR差分を取得
- `post_review_comment(comment)`: レビューコメントを投稿

**共通ツール**（`ChatClientAgentOptions.tools`に登録）:
- GitLab API（MR差分取得、コメント投稿）
- コードベース検索ツール
...
**Agent Frameworkクラス**: `CodeGenerationPlanningAgent`（`BasePlanningAgent`を継承）

**責務**: コード生成タスクの実行計画を生成する。プロンプト詳細はPROMPTS.mdを参照。

**実装方法**:
- `BasePlanningAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（コンテキスト取得、仕様書確認、Todoリスト作成等）を利用
- コード生成特有のプランニングロジックを実装
...
**Agent Frameworkクラス**: `BugFixPlanningAgent`（`BasePlanningAgent`を継承）

**責務**: バグ修正タスクの実行計画を生成する。プロンプト詳細はPROMPTS.mdを参照。

**実装方法**:
- `BasePlanningAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（コンテキスト取得、仕様書確認、Todoリスト作成等）を利用
- バグ修正特有のプランニングロジック（エラー解析、根本原因特定）を実装
...
**Agent Frameworkクラス**: `TestCreationPlanningAgent`（`BasePlanningAgent`を継承）

**責務**: テスト作成タスクの実行計画を生成する。プロンプト詳細はPROMPTS.mdを参照。

**実装方法**:
- `BasePlanningAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（コンテキスト取得、仕様書確認、Todoリスト作成等）を利用
- テスト作成特有のプランニングロジック（テスト戦略決定、テストケース設計）を実装
...
**Agent Frameworkクラス**: `DocumentationPlanningAgent`（`BasePlanningAgent`を継承）

**責務**: ドキュメント生成タスクの実行計画を生成する。プロンプト詳細はPROMPTS.mdを参照。

**実装方法**:
- `BasePlanningAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（コンテキスト取得、仕様書確認、Todoリスト作成等）を利用
- ドキュメント生成特有のプランニングロジック（ドキュメント構成決定、対象読者特定）を実装
...
**Agent Frameworkクラス**: `PlanReflectionAgent`（`BaseReflectionAgent`を継承）

**責務**: プランニング後にプランを検証し、問題点を特定して改善案を提示する。プロンプト詳細はPROMPTS.mdを参照。

**実装方法**:
- `BaseReflectionAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（プラン取得、問題点特定、改善提案生成等）を利用
- プラン検証特有のロジック（整合性チェック、完全性評価、実現可能性評価）を実装
...
**Agent Frameworkクラス**: `CodeGenerationAgent`（`BaseExecutionAgent`を継承）

**責務**: 新規コードを生成する

**実装方法**:
- `BaseExecutionAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（プラン取得、ファイル操作、実行結果記録、git操作）を利用
- コード生成特有の実行ロジック（仕様書理解、設計の詳細化、デザインパターン適用）を実装
...
**Agent Frameworkクラス**: `BugFixAgent`（`BaseExecutionAgent`を継承）
...
- `BaseExecutionAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（プラン取得、ファイル操作、実行結果記録、git操作）を利用
- バグ修正特有の実行ロジック（根本原因特定、最小限の変更、リグレッション防止）を実装
...
**Agent Frameworkクラス**: `DocumentationAgent`（`BaseExecutionAgent`を継承）

**責務**: ドキュメントを作成する

**実装方法**:
- `BaseExecutionAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（プラン取得、ファイル操作、実行結果記録）を利用
- ドキュメント生成特有の実行ロジック（Markdown記述、Mermaid図作成、用語統一）を実装
...
**Agent Frameworkクラス**: `TestCreationAgent`（`BaseExecutionAgent`を継承）
...
- `BaseExecutionAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（プラン取得、ファイル操作、実行結果記録、git操作）を利用
- テスト作成特有の実行ロジック（テストフレームワーク選択、モック/スタブ作成、カバレッジ測定）を実装
...
**Agent Frameworkクラス**: `TestExecutionEvaluationAgent`（`BaseReviewAgent`を継承）

**責務**: テストコードを実行し、結果を評価する

**実装方法**:
- `BaseReviewAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（差分取得、レビュー実行、コメント生成、GitLab投稿、結果記録）を利用
- テスト評価特有のレビューロジック（成功率評価、カバレッジ分析、失敗原因特定）を実装
...
**Agent Frameworkクラス**: `CodeReviewAgent`（`BaseReviewAgent`を継承）
...
- `BaseReviewAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（差分取得、レビュー実行、コメント生成、GitLab投稿、結果記録）を利用
- コードレビュー特有のレビューロジック（品質チェック、ロジックレビュー、セキュリティチェック）を実装
...
LLMへ渡すロール定義・チェック項目・出力フォーマットのプロンプト詳細はPROMPTS.mdを参照

#### Documentation Review Agent

**Agent Frameworkクラス**: `DocumentationReviewAgent`（`BaseReviewAgent`を継承）
...
- `BaseReviewAgent`を継承し、`AIAgent`を内部で使用
- 親クラスの共通機能（差分取得、レビュー実行、コメント生成、GitLab投稿、結果記録）を利用
- ドキュメントレビュー特有のレビューロジック（正確性チェック、構造チェック、完全性チェック）を実装
...
- `WorkflowBuilder`でプランニング・実行・検証の各フェーズをステップとして登録する
- 各ステップは`Executor`または`AIAgent`で実装する
...
flowchart TD
    ...
    TaskType -->|コード生成| CodeGenPlanning[Code Generation<br/>Planning Agent]
    TaskType -->|バグ修正| BugFixPlanning[Bug Fix<br/>Planning Agent]
    TaskType -->|ドキュメント生成| DocPlanning[Documentation<br/>Planning Agent]
    TaskType -->|テスト作成| TestPlanning[Test Creation<br/>Planning Agent]
    ...
    CodeReview --> Reflection[リフレクション<br/>タスク種別別Planning Agent]
...
| 計画 | Code Generation/Bug Fix/Test Creation/Documentation Planning Agent | タスク種別別実行プラン生成 |
...
| リフレクション | タスク種別別Planning Agent | 結果評価・再計画判断 |

**重要なフロー特性**:

1. **仕様ファイル必須（コード生成系）**: コード生成、バグ修正、テスト作成で仕様ファイルがなければドキュメント生成計画を立案し、仕様書作成後にタスク完了
2. **タスク種別別Planning Agent**: 各タスク種別に専用のPlanning Agentを使用（4種類）
...
**使用エージェント**: タスク種別別Planning Agent（Code Generation Planning Agent / Bug Fix Planning Agent / Test Creation Planning Agent / Documentation Planning Agent）
...
- Documentation Planning Agentがドキュメント生成のための計画を立案
- Documentation Agent → Documentation Review Agent で仕様書を作成
- リフレクション（Documentation Planning Agent）で問題があれば再計画
...
**使用エージェント**: タスク種別別Planning Agent（実行中のタスクに応じたPlanning Agent）
...
```mermaid
flowchart TD
    Start[コード生成タスク開始] --> Planning[Code Generation<br/>Planning Agent<br/>実行計画生成]
...
**フロー詳細**:
1. Code Generation Planning Agentが実行計画を生成
2. 関連する仕様ファイルの存在確認
3. **仕様ファイルがない場合**:
   - Documentation Planning Agentがドキュメント生成計画を立案
...
```mermaid
flowchart TD
    Start[バグ修正タスク開始] --> Planning[Bug Fix<br/>Planning Agent<br/>実行計画生成]
    Planning --> SpecCheck{仕様ファイル確認}
...
**フロー詳細**:
1. Bug Fix Planning Agentが実行計画を生成
...
```mermaid
flowchart TD
    Start[ドキュメント生成タスク開始] --> Planning[Documentation<br/>Planning Agent<br/>実行計画生成]
...
**フロー詳細**:
1. Documentation Planning Agentが実行計画を生成
...
```mermaid
flowchart TD
    Start[テスト作成タスク開始] --> Planning[Test Creation<br/>Planning Agent<br/>実行計画生成]
    Planning --> SpecCheck{仕様ファイル確認}
...
**フロー詳細**:
1. Test Creation Planning Agentが実行計画を生成
...
classDiagram
    ...
    class BasePlanningAgent {
        <<abstract>>
        -CodebaseSearchTool search_tool
        -TodoManager todo_manager
        +plan_async()* PlanResult
        #create_action_plan() Plan
        #verify_spec_exists() bool
    }
    ...
    class BaseReflectionAgent {
        ...
        -CodebaseSearchTool search_tool
        -TodoManager todo_manager
        +reflect_async()* ReflectionResult
        #get_plan() Plan
        #identify_issues(plan) List~Issue~
        #generate_improvements(issues) List~Improvement~
    }
    ...
    class BaseReviewAgent {
        <<abstract>>
        -GitLabClient gitlab_client
        +review_async()* ReviewResult
        #get_mr_diff() Diff
        #post_review_comment(comment) void
    }
    ...
    class AgentFactory {
        ...
        -PromptOverrideManager prompt_override_manager
        +create_planning_agent(type, session_id, user_id) BasePlanningAgent
        +create_reflection_agent(session_id, user_id) BaseReflectionAgent
        +create_execution_agent(type, session_id, user_id) BaseExecutionAgent
        +create_review_agent(type, session_id, user_id) BaseReviewAgent
        -_create_session(session_id) AgentSession
        -_get_prompt(agent_name, user_id) str
        -_register_tools(agent_type) List~Tool~
    }
    ...
    WorkflowFactory --> PromptOverrideManager : 保持
    ...
    Workflow "1" *-- "n" BaseAgent : 複数保持
    Workflow "1" *-- "n" TokenUsageMiddleware : 複数保持
    
    BaseAgent --> AgentSession : 保持
    ...
    BaseAgent --> IWorkflowContext : 参照
    BaseAgent --> MCPClientRegistry : 参照
    
    BasePlanningAgent --|> BaseAgent : 継承
    BaseReflectionAgent --|> BaseAgent : 継承
    BaseExecutionAgent --|> BaseAgent : 継承
    BaseReviewAgent --|> BaseAgent : 継承
    
    UserResolverExecutor --|> BaseExecutor : 継承
    ContentTransferExecutor --|> BaseExecutor : 継承
    EnvironmentSetupExecutor --|> BaseExecutor : 継承
    
    BasePlanningAgent --> TodoManager : 参照
    BaseReflectionAgent --> TodoManager : 参照
    BaseExecutionAgent --> ExecutionEnvironmentManager : 参照
    BaseReviewAgent --> GitLabClient : 参照
    ...
    AgentFactory --> BaseAgent : 生成
    AgentFactory --> PromptOverrideManager : 使用
    
    TokenUsageMiddleware --> ContextStorageManager : 使用
    
    PromptOverrideManager --> ContextStorageManager : 使用（agent_prompt_overridesテーブル）
...
4. `WorkflowFactory`の初期化
   ...
   - `PromptOverrideManager`のインスタンス化（agent_prompt_overridesテーブルとPROMPTS.mdからプロンプトロード）
...
3. `WorkflowExecutor.execute_workflow(task_type, user_id, context)`でワークフロー実行開始
4. `WorkflowFactory`がタスク種別に応じたWorkflowを生成
   - `create_code_generation_workflow(user_id)`: コード生成ワークフロー
   - `create_bug_fix_workflow(user_id)`: バグ修正ワークフロー
   - `create_test_generation_workflow(user_id)`: テスト生成ワークフロー
   - `create_refactoring_workflow(user_id)`: リファクタリングワークフロー
   - `create_document_workflow(user_id)`: ドキュメント生成ワークフロー
   - `create_code_review_workflow(user_id)`: コードレビューワークフロー
5. `WorkflowBuilder`が`Workflow`オブジェクトを構築
   - `IWorkflowContext`を生成（タスク専用、セッションID付き）
   - `ExecutorFactory`から必要な`Executor`を生成して追加
   - `AgentFactory`から必要な`AIAgent`を生成して追加
...
7. `AgentFactory`が各`AIAgent`を生成
   - `PromptOverrideManager`からユーザー固有のプロンプトを取得（存在しない場合はPROMPTS.mdのデフォルト）
...
- Consumer起動時に1回初期化
- `agent_prompt_overrides`テーブルからユーザー固有のプロンプトをロード
- PROMPTS.mdからデフォルトプロンプトをロード
- プロンプト取得優先順位: 1) ユーザー固有プロンプト、2) デフォルトプロンプト
...
sequenceDiagram
    ...
    participant BPA as BasePlanningAgent
    ...
    participant BEA as BaseExecutionAgent
    participant MCPR as MCPClientRegistry
    participant MCPC as MCPClient
    participant EEM as ExecutionEnvironmentManager
    participant BRA as BaseReviewAgent
    
    Note over C,TH: 1. タスク開始フロー
    C->>TH: タスク受信
    TH->>TSF: create_strategy(task)
    TSF-->>TH: タスク戦略
    TH->>WE: execute_workflow(task_type, user_id, context)
    WE->>WF: create_*_workflow(user_id)
    WF->>WB: Workflowビルド開始
    WB->>WB: ExecutorFactory.create_*_executor(context)
    WB->>WB: AgentFactory.create_*_agent(type, session_id, user_id)
    WB->>W: Workflowオブジェクト生成
    W->>IWC: コンテキスト初期化
    IWC->>CSM: コンテキスト初期化
    
    Note over W,PR: 2. プランニングフェーズ
    ...
    PCP-->>AS: プランニングコンテキスト
    BPA->>LLM: プロンプト送信（履歴+コンテキスト）
    LLM-->>BPA: プラン生成結果
    BPA->>TUM: on_agent_completed(tokens)
    TUM->>CSM: トークン使用量記録(user_id, task_uuid, tokens)
    BPA->>TM: create_todo_list()
    BPA->>PCP: store_plan_async()
    PCP->>CSM: save_planning_history()
    BPA->>PCHP: store_message_async()
    PCHP->>CSM: 会話履歴保存
    BPA->>IWC: queue_state_update_async()
    BPA->>GL: プラン投稿
    BPA->>PR: report_phase_complete()
    
    Note over W,PR: 3. 実行フェーズ
    W->>TUM: on_agent_invoked()
    W->>BEA: execute_async()
    BEA->>AS: 履歴とコンテキスト取得
    AS->>PCHP: provide_history_async()
    ...
    TRCP-->>AS: ツール実行結果
    BEA->>MCPR: get_client("text-editor")
    MCPR-->>BEA: MCPClient
    BEA->>MCPC: invoke_tool("create_file", params)
    MCPC-->>BEA: 実行結果
    BEA->>TUM: on_agent_completed(tokens)
    TUM->>CSM: トークン使用量記録(user_id, task_uuid, tokens)
    BEA->>TRCP: store_result_async()
    TRCP->>CSM: save_tool_result()
    BEA->>EEM: execute_command("git add ...")
    BEA->>PCHP: store_message_async()
    PCHP->>CSM: 会話履歴保存
    BEA->>IWC: queue_state_update_async()
    BEA->>TM: update_todo_status("completed")
    BEA->>GL: 進捗投稿
    BEA->>PR: report_phase_complete()
    
    Note over W,PR: 4. レビューフェーズ
    W->>TUM: on_agent_invoked()
    W->>BRA: review_async()
    BRA->>AS: 履歴とコンテキスト取得
    AS->>PCHP: provide_history_async()
    ...
    BRA->>GL: get_mr_diff()
    GL-->>BRA: MR差分
    BRA->>LLM: レビュー実行（履歴+コンテキスト）
    LLM-->>BRA: レビュー結果
    BRA->>TUM: on_agent_completed(tokens)
    TUM->>CSM: トークン使用量記録(user_id, task_uuid, tokens)
    BRA->>GL: post_comment()
    BRA->>PCHP: store_message_async()
    PCHP->>CSM: 会話履歴保存
    BRA->>IWC: queue_state_update_async()
    BRA->>PR: report_phase_complete()
    
    Note over W,PR: 5. エラーハンドリング
    alt エラー発生
        BEA->>TUM: on_agent_completed(tokens, error)
        TUM->>CSM: トークン使用量記録
        BEA->>PCHP: store_message_async(error)
        BEA->>IWC: queue_state_update_async(error)
        BEA->>PR: report_error()

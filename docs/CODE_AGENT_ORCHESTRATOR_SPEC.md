...
|------------|------|---------|---------------------|
| **IssueToMRConverter** | Issue→MR変換 | `Workflow` + `Executor` | coding_agentを参考にして、`WorkflowBuilder`でワークフローを構築、`Executor`で各ステップを実装 |
| **Branch Naming Agent** | ブランチ名生成 | `AIAgent` | coding_agentを参考にして、`ChatClient.as_ai_agent()`でAIAgentを生成、LLMにブランチ名生成を依頼 |

#### 2.3.3 MR処理 Layer（本フロー - Agent Framework）

| コンポーネント | 責務 | Agent Frameworkクラス | coding_agent参照・実装方針 |
|------------|------|---------|---------------------|
| **PlanningCoordinator** | 計画実行制御 | `Workflow` + `Executor` | `WorkflowBuilder`でプランニング・実行・検証のフローを構築、`Executor`で各フェーズを実装 |
| **TextEditorMCPClient** | ファイル編集 | MCPツール登録 | Agent Frameworkのツール登録機能でMCP Serverを統合 |
| **ExecutionEnvironmentManager** | Docker環境管理 | `Executor` | `handlers/execution_environment_manager.py` 参考、`Executor`として実装 |
| **AIAgent (プランニング/実行/レビュー)** | LLM呼び出し | `AIAgent` | `clients/llm_*.py` 参考、Agent Frameworkの`Agent Providers`を使用して`ChatClient.as_ai_agent()`でAIAgentを生成 |

**実装方針**: カスタムツール（todo管理等）はMCPではなくAgent Frameworkのツールとして実装する。
...
**ラベル仕様**: issueのラベルはcoding_agentの慣習に準拠する。coding_agentの`task_label`（処理対象ラベル）、`processing_label`（処理中ラベル）、`done_label`（完了ラベル）と同じラベル名を使用する。
...
4. タスクに完了ラベル（done）を付与する

**ラベル仕様**: coding_agentの慣習に準拠する（task_label, processing_label, done_label等）。
...
Agent Frameworkの`AIAgent`を基底として、以下の3つの抽象基底クラスを定義し、各具体的エージェントはこれらを継承する。

```
AIAgent (Agent Framework標準)
...
**共通機能**:
1. **コンテキスト取得**: `IWorkflowContext`から要件情報、プロジェクト情報を取得
...
6. **コンテキスト保存**: `IWorkflowContext.queue_state_update_async()`でプランを保存

**共通ツール**（`ChatClientAgentOptions.tools`に登録）:
- コードベース検索ツール
- ファイル読み込みツール
- `create_todo_list`ツール
- `sync_to_gitlab`ツール

**実装方法**:
- Agent Providersの`ChatClient.as_ai_agent()`で生成した`AIAgent`をラップする
...
**共通機能**:
1. **プラン取得**: `IWorkflowContext`から実行計画を取得
2. **コード/ドキュメント生成**: LLMに生成依頼
3. **ファイル操作**: MCP Server (text-editor)で編集
4. **実行結果記録**: `IWorkflowContext.queue_state_update_async()`で結果を保存
...
5. **結果記録**: `IWorkflowContext.queue_state_update_async()`で結果を保存
...
- 取得したAPIキーをワークフローコンテキストに保存する（`IWorkflowContext.queue_state_update_async()`）

**処理フロー**:
1. ワークフローからメールアドレスを受け取る
2. User Config APIに問い合わせる (GET /api/v1/config/{email})
3. ユーザーが未登録の場合、例外をスローしてワークフローを停止する
4. APIキーを復号化してワークフローコンテキストに保存する
5. 後続のExecutor/AIAgentがこのAPIキーを使用してLLMにアクセスする
...
- `Executor`: 各ステップの実行ロジック
- `WorkflowBuilder`: ワークフローの構築
- `IWorkflowContext`: ステップ間のデータ共有

**実装方法**:
coding_agentを参考にして、Agent Frameworkのワークフローとして実装する。clients/gitlab_client.pyはそのまま流用する。

| コンポーネント | 参照元 | 役割 | Agent Frameworkクラス |
|----------------|-----------|------|---------|
| **IssueToMRConverter** | `handlers/issue_to_mr_converter.py` | Issue→MR変換のメインクラス | `Workflow`を使用して定義 |
| **BranchNameGenerator** | `handlers/issue_to_mr_converter.py` | LLMを使用したブランチ名生成 | `AIAgent`で実装 |
| **ContentTransferManager** | `handlers/issue_to_mr_converter.py` | IssueコメントのMRへの転記 | `Executor`で実装 |
| **GitlabClient** | `clients/gitlab_client.py` | GitLab API操作（ブランチ、コミット、MR作成） | `Executor`のツールとして登録 |
...
- **`provide_ai_context_async(context, cancellation_token)`**
  - Agent Frameworkがエージェント実行前に呼び出すメソッド
  - `context`からタスクUUIDを取得する
  - ファイルストレージから当該タスクの過去のツール実行結果を読み込む
...
- **`store_ai_context_async(context, cancellation_token)`**
  ...
  - ファイル読み込みツールの場合、ファイルパス、内容プレビュー、MIMEタイプをJSON形式で保存する
  - コマンド実行ツールの場合、コマンド、終了コード、標準出力、標準エラー出力をJSON形式で保存する
  - MCPツール呼び出しの場合、ツール名、引数、結果をJSON形式で保存する
  - タイムスタンプ付きのファイル名で`tool_results/{task_uuid}/`ディレクトリに保存する
  - `metadata.json`にツール実行統計情報を更新する
```
...
4. **セッションの永続化**
   ...
   - シリアル化されたJSONをPostgreSQLまたはRedisに保存する
   - タスク一時停止や再起動時に使用する
...
| ChatHistoryProvider | 会話履歴永続化 | PostgreSQL等にカスタム実装で永続化可能 |
| AIContextProvider | コンテキスト・メモリ管理 | プランニング履歴、ツール結果のコンテキスト統合 |
| ProviderSessionState<T> | セッション状態管理 | 型安全なセッション情報管理 |
...
#### 14.3.2 コンテキスト管理（Agent Framework Providerパターンで再実装）

| ファイルパス | 主要クラス/関数 | 用途 | 移植方法 | 変更点 |
|------------|---------------|------|---------|--------|
| **`context_storage/task_context_manager.py`** | `TaskContextManager` | タスクコンテキスト管理 | **Agent Framework Providerで再実装** | **ChatHistoryProvider + AIContextProviderで置き換え** |
| **`context_storage/message_store.py`** | `MessageStore` | LLM会話履歴保存 | **PostgreSqlChatHistoryProviderで再実装** | **Agent Framework標準パターンに合わせる** |
| **`context_storage/summary_store.py`** | `SummaryStore` | 要約保存 | **PlanningContextProviderに統合** | **AIContextProviderとして実装** |
| **`context_storage/tool_store.py`** | `ToolStore` | ツール実行履歴 | **ToolResultContextProviderで再実装** | **AIContextProviderとして実装** |
| **`context_storage/context_compressor.py`** | `ContextCompressor` | コンテキスト圧縮 | 参考にして再実装 | LLMは Agent Framework経由 |
| **`context_storage/context_inheritance_manager.py`** | `ContextInheritanceManager` | コンテキスト継承 | 参考にして再実装 | AIContextProviderで実装 |

**重要**: coding_agentのファイルベースのコンテキスト管理を、Agent Framework標準の`ChatHistoryProvider`と`AIContextProvider`パターンで再設計する。これによりフレームワーク標準に準拠した実装となり、保守性が向上する。

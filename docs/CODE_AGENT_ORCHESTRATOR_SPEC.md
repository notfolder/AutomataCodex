...
6. 各Executorは共通のIWorkflowContextを介してタスク全体の状態を共有
...
**保持する依存性**:
- `agent`: ChatCompletionAgentインスタンス（Agent Framework提供）
- `kernel`: Kernelインスタンス（ツール管理）
...
`ConcurrentOrchestration`（semantic-kernel python-1.40.0時点でexperimental）は`InProcessRuntime`上のasyncioコルーチン並行処理として動作する。OSスレッド並列ではないが、`await`をまたぐ操作の間に他のコルーチンが同じ`execution_environments`辞書を更新するインターリービングが発生しうる。このため本システムが自前でスレッドセーフ性を担保する。具体的な実装方針：
...
1. **ConfigurableAgentの書き込み処理**:
   ...
   - `asyncio.Lock`を使用して排他制御を行い、複数コルーチンからの同時書き込みを防ぐ

2. **asyncio.Lockによる排他制御**:
   - ワークフロー開始時に`asyncio.Lock`インスタンスを生成し、コンテキスト経由でエージェントに共有する
   - `execution_environments`辞書への書き込み前後でロックを取得・解放する
   - ロック保持中は`await`を含む処理を行わず、クリティカルセクションを最小化する

3. **実装時の注意点**:
   - `ConcurrentOrchestration`はexperimentalのため、SK バージョンアップ時に破壊的変更が入るリスクに注意する
   - テスト時に`asyncio.gather`で複数エージェントを並列実行し、`execution_environments`へのインターリービングが発生しないことを確認する
...
- 取得したOpenAI APIキーをワークフローコンテキストに保存する（`IWorkflowContext` の `set_state()`メソッドに`asyncio.Lock`で排他制御して書き込み）
...
graph TB
    ...
    subgraph "エージェント実装層"
        Agent1[ChatCompletionAgent<br/>計画エージェント]
        Agent2[ChatCompletionAgent<br/>コード生成エージェント]
        Agent3[ChatCompletionAgent<br/>レビューエージェント]
        Agent4[ChatCompletionAgent<br/>テストエージェント]
        Agent5[ChatCompletionAgent<br/>バグ修正エージェント]
        Agent6[ChatCompletionAgent<br/>差分計画エージェント]
        Agent7[ChatCompletionAgent<br/>リフレクションエージェント]
        AgentN[ChatCompletionAgent<br/>その他エージェント]
...
- **PlanningContextProvider**: プランニング履歴をPostgreSQLに永続化（Agent Framework標準AIContextProviderを継承）
- **ToolResultContextProvider**: ツール実行結果をファイル+PostgreSQLに永続化（Agent Framework標準AIContextProviderを継承）
- **TaskInheritanceContextProvider**: 同一Issue/MRの過去タスクから継承データを提供（Agent Framework標準AIContextProviderを継承）
...
Agent Frameworkの`IWorkflowContext`を介してワークフロー全体で共有されるデータのキー定義。すべてのキーはスコープ`workflow`で保存される。
...
同一Issue/MRに対する過去のタスク実行履歴から有用な情報を抽出し、新しいタスクのコンテキストとして提供する。継承処理は**TaskInheritanceContextProvider**が担当し、Agent FrameworkのAIContextProviderとして実装される。
...
TaskInheritanceContextProvider.provide_ai_context_async()メソッドが、過去タスクのInheritanceDataを取得し、Markdown形式に整形してAIContextとして返す。
...
**本システムが独自実装する `IWorkflowContext` インターフェース**を通じて複数のExecutor間で共有状態を管理する。Agent Framework（semantic-kernel）は対応するインターフェースを提供しないため、本システムが `asyncio.Lock` を内包した独自クラスとして実装する。

**状態の書き込み処理**:

- Executor内で`context.set_state()`メソッドを呼び出す
- 引数として以下を指定する：
  - `key`: 状態を識別するキー（文字列）
  - `value`: 保存する値（任意の型、JSON serializable）
  - `scope_name`: 状態のスコープ名（名前空間として機能）
- 内部で`asyncio.Lock`を取得してから辞書への書き込みを行い、完了後にロックを解放する
- 同じscope_name内では一意のkeyで状態を管理する

**状態の読み込み処理**:

- 別のExecutor内で`context.get_state()`メソッドを呼び出す
- 引数として以下を指定する：
  - `key`: 取得したい状態のキー（文字列）
...
**継承元**: `ChatCompletionAgent`（Agent Framework標準クラス）
...
- `kernel`: Agent FrameworkのKernelインスタンス
- `config`: 学習機能の設定
...
`invoke_async(context)`メソッドで以下を実行する：
...
5. **LLM判断**: ChatCompletionAgent標準機能を使用してLLMに単一呼び出し
...
### 15.2 Microsoft Semantic Kernel (Python)標準機能の活用

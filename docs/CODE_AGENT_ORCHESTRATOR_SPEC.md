...
- **[Microsoft Agent Framework](https://github.com/microsoft/agent-framework)（Python）**: エージェントオーケストレーション基盤（MR処理のみ）
  - **[Filters (Middleware)](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)**: リクエスト/レスポンス処理、例外ハンドリング、カスタムパイプライン
  - **[OpenTelemetry統合](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)**: 分散トレーシング、モニタリング、デバッグ
  - **[AI Services](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)**: 複数のLLMプロバイダーサポート
  - **[Process Framework (Workflow/Executor)](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)**: ワークフロー実行とExecutor管理
...
| **Branch Naming Agent** | ブランチ名生成 | [`Agent`](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) | [coding_agent](https://github.com/notfolder/coding_agent)を参考にして、Agent FrameworkのAgentを使用してLLMにブランチ名生成を依頼 |
...
| **ConfigurableAgent** | エージェントノード実行 | [`Executor`](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) | エージェント定義とプロンプト定義に基づいて動作する汎用エージェント |
...
| **PostgreSqlChatHistoryProvider** | 会話履歴永永化 | PostgreSQL | Agent Framework [BaseHistoryProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)パターン | [context_storage/message_store.py](https://github.com/notfolder/coding_agent/blob/main/context_storage/message_store.py)参考、独自のHistory管理クラス実装 |
...
- `_setup_environments(graph_def)`: グラフ定義内でenvironment_mode="create"のノード分のDocker環境を準備する
...
**継承元**: [`Executor`](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)
...
- `environment_mode`: 実行環境（Docker）の使用方法（"create": 新規作成、"inherit": 引き継ぎ、"none": 不要）
- `prompt_id`: プロンプト定義ファイル内のプロンプト識別子

**保持する依存性**:
- `agent`: Agentインスタンス（Agent Framework提供）
- `progress_reporter`: ProgressReporterインスタンス（進捗報告機能）
- `environment_id`: Docker環境ID（environment_mode="create"または"inherit"の場合）
...
- `agent_definition_id`: エージェントの定義を識別（例: code_generation_fast、code_generation_standard）。ワークフローコンテキストの`execution_environments`辞書のキーとして使用
...
- `get_environment_id()`: ワークフローコンテキストの`execution_environments`辞書をチェックし、当該`agent_definition_id`の環境IDが存在する場合はそれを返す。存在しない場合はExecutionEnvironmentManagerから`node_id`を使用して環境プールから未使用の環境IDを割り当て、`execution_environments`辞書に記録する。環境自体はワークフロー開始時に事前作成済み
- `report_progress(phase, message, details)`: 進捗状況をMRコメントとして投稿する（内部でProgressReporterを使用）

**ロール別の処理内容**:
- **planning**: コンテキスト取得（`task_mr_iid`含む）→進捗報告（開始）→LLM呼び出し（プランニング）→進捗報告（LLM応答）→Todoリスト作成→進捗報告（計画完了）→コンテキスト保存
- **reflection**: プラン取得→進捗報告（開始）→LLM呼び出し（検証）→進捗報告（LLM応答）→改善判定→進捗報告（検証完了）→コンテキスト保存
- **execution**: プラン取得→環境ID取得（`get_environment_id()`で`execution_environments`から引き継ぎまたは`node_id`で環境プールから割り当て）→進捗報告（開始）→LLM呼び出し（実装/生成）→進捗報告（LLM応答）→ファイル操作（MCPツール）→git操作→進捗報告（実行完了）→環境プールから割り当てた場合は`execution_environments[agent_definition_id] = env_id`として保存→コンテキスト保存
- **review**: MR差分取得→`execution_environments`辞書から対応する実行エージェントの環境IDを取得（`get_environment_id()`で自動引き継ぎ）→進捗報告（開始）→LLM呼び出し（レビュー）→進捗報告（LLM応答）→コメント生成→進捗報告（レビュー完了）→コンテキスト保存

**進捗報告の実装方針**:

各エージェントは実行の各フェーズで`report_progress()`メソッドを呼び出し、進捗状況をMRコメントとして投稿する。具体的な呼び出しタイミング：

1. **処理開始時**: ワークフローコンテキストから`task_mr_iid`を取得し、`report_progress("${role}_start", "処理を開始します", details)`を呼び出す
2. **LLM応答後**: LLM呼び出しが完了したら、`report_progress("${role}_llm_response", "LLM応答を取得しました", {summary: response_summary})`を呼び出す
3. **処理完了時**: ロール別の後処理が完了したら、`report_progress("${role}_complete", "処理が完了しました", details)`を呼び出す

ProgressReporterは内部で`format_progress_comment()`を使用してフェーズに応じた適切なMarkdown形式のコメントを生成し、GitLab APIを介してMRに投稿する。

**ワークフローコンテキストへの並列書き込み対応**:

並列実行時に複数のエージェントが同時に`execution_environments`辞書へ書き込む場合、Agent FrameworkのBSP（Bulk Synchronous Parallel）モデルによりデータ競合は発生しない。具体的な実装方針：

1. **ConfigurableAgentの書き込み処理**:
   - 実行エージェントは環境ID取得後、`execution_environments`辞書の該当キー（`agent_definition_id`）のみを更新
...
##### Environment Setup Executor

**Agent Frameworkクラス**: `Executor`

**責務**: グラフ定義で`environment_mode: "create"`のノード分のDocker環境を準備する

**処理フロー**:
1. ワークフローコンテキストから`task_mr_iid`を取得
2. グラフ定義から`environment_mode: "create"`のノードをフィルタリング
3. フィルタリングしたノードのnode_idリストを取得
4. ワークフローコンテキストから`selected_environment`（環境名）を取得
5. ExecutionEnvironmentManager.prepare_environments(count=ノード数, environment_name=選択された環境名, mr_iid=MR IID, node_ids=ノードIDリスト)を呼び出し
6. 返された環境IDリストを確認（各環境IDは`codeagent-{environment_name}-mr{mr_iid}-{node_id}`形式）
7. 環境準備完了をワークフローコンテキストに記録
8. 各実行エージェントがget_environment_id()を呼び出して環境プールから環境IDを割り当てる
...
- `multi_codegen_mr_processing`: 複数コード生成並列実行（3種類のモデル・温度設定で並列実行し、レビュー時に自動選択）
  ...
  - 各エージェントは専用ブランチ（例: `feature/login-fast`, `feature/login-standard`, `feature/login-creative`）で作業し、レビューエージェントが最良のブランチを自動選択してマージ
...
7. **environment_mode集計**:
   - `environment_mode: "create"`のノード数を数えて返す
   - この数値は`WorkflowFactory._setup_environments()`で必要なDocker環境数の根拠となる
   - `environment_mode: "inherit"`のノード（環境引き継ぎ）は環境数に含めない

**戻り値**: 検証成功時は`environment_mode: "create"`のノード数を返す。検証失敗時は`DefinitionValidationError`例外をスローする。
...
6. **output_keysの一意性（並列実行対応）**:
   - 単一エージェントも並列エージェントも同じ辞書型キー（`execution_environments`、`execution_results`）への書き込みは複数エージェントで共有可能
...
- `_setup_environments(graph_def)`: グラフ定義内で`environment_mode: "create"`のノードに対してDocker環境を準備する（Agent FrameworkのExecutorとして実装）

**複数環境のサポート**:
グラフ定義でノードごとに`environment_mode`を設定することで、必要なノード分の実行環境を事前に作成する。これにより、複数のコーディングエージェントノードが独立した環境で並列実行可能になる。

**環境準備と割り当ての詳細**:
1. **環境数の決定**: `validate_graph_definition()` が `environment_mode: "create"` のノードを集計し、WorkflowFactory がその数分のDocker環境を準備する。`environment_mode: "inherit"`のノード（レビューエージェント等）はカウントしない
2. **環境プール管理**: ExecutionEnvironmentManager が環境プールを管理。`environment_mode: "create"`のノードは環境プールから独自の環境IDを取得し、`environment_mode: "inherit"`のノードは`execution_environments`コンテキストから既存の環境IDを読み取って使用する
...
**例**: `multi_codegen_mr_processing` プリセットでは、3つの並列コード生成ノード（code_generation_fast、code_generation_standard、code_generation_creative）がそれぞれ独自の環境を必要とするため、合計3つのDocker環境が事前に準備される。test_execution_evaluationノードはcode_generationノードの環境を引き継ぐため、新規環境は必要としない。

### 4.5 マルチエージェント実行時のブランチ管理（multi_codegen_mr_processing）

`multi_codegen_mr_processing`プリセットでは、複数のコード生成エージェントが並列実行され、各エージェントが独立したブランチで作業を行う。

#### 4.5.1 ブランチ作成戦略

**ワークフロー開始時の処理**:

1. **元MRブランチの取得**: タスクのMR情報から元のブランチ名（例: `feature/login`）を取得
2. **サブブランチの作成**: 各並列エージェント用にサブブランチを作成
   ```
   feature/login (元MRブランチ)
   ...
   ```
3. **ブランチ名の生成**: `{元ブランチ名}-{エージェントID末尾}`形式（例: `feature/login-fast`）
4. **task_contextへの追加**: 各エージェントの`task_context`に`assigned_branch`フィールドを追加し、担当ブランチ名を設定

**実装箇所**: 
- `WorkflowFactory._setup_parallel_branches(graph_def, task_context)`: 並列実行ノードのブランチを事前作成
- `GitLabClient.create_branch(branch_name, base_branch)`: 既存のブランチ作成APIを使用（新規実装不要）

#### 4.5.2 各エージェントの作業フロー

各並列エージェント（code_generation_fast、code_generation_standard、code_generation_creative）は以下のフローで作業：
...
- 各エージェントは独立したDocker環境で動作し、ブランチも分離されているため競合は発生しない

#### 4.5.3 レビューと自動選択

**レビューフェーズ**:

1. **3つの成果物を一括レビュー**: `code_review`エージェント（`code_review_multi`プロンプト使用）が以下を実行
   - 3つの`execution_result`（fast、standard、creative）を比較
...
3. **マージ処理**: 
   - 選択されたブランチ（例: `feature/login-standard`）を元MRブランチ（`feature/login`）にマージ
...
**すべてのブランチを保持**:
- 選択されなかったブランチ（例: `feature/login-fast`、`feature/login-creative`）もGitLab上に残す
...
#### 4.5.5 データベーススキーマへの影響

`tasks`テーブルに以下のカラムを追加：

```sql
ALTER TABLE tasks ADD COLUMN assigned_branches JSONB;
-- 例: {"fast": "feature/login-fast", "standard": "feature/login-standard", "creative": "feature/login-creative"}

ALTER TABLE tasks ADD COLUMN selected_branch VARCHAR(255);
-- レビュー後に選択されたブランチ名を記録
```
...
| **BranchNameGenerator** | `handlers/issue_to_mr_converter.py` | LLMを使用したブランチ名生成 | [`Agent`](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)で実装 |
...
**主要な設計パターン**:
1. **Provider方式によるコンテキスト管理**: `PostgreSqlChatHistoryProvider`、`PlanningContextProvider`、`ToolResultContextProvider`を使用し、Agent Framework [BaseHistoryProvider/BaseContextProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)パターンでコンテキストを永続化する
2. **Filters方式によるトークン統計**: `TokenUsageMiddleware`を使用し、すべての[`Agent`](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)呼び出しを自動的にインターセプトしてトークン使用量を記録する
...
graph TB
    ...
    subgraph "Executor層"
        ...
        EnvironmentSetupExec[EnvironmentSetupExecutor<br/>環境セットアップ]
...
| `execution_environments` | dict | エージェント定義IDをキーとした環境ID辞書 | executionエージェント実行時 |
...
### 8.3 Agent Framework標準Providerのカスタム実装
...
### 8.7 ワークフロー状態管理（WorkflowContext）
...
`ExecutionEnvironmentManager`はDocker環境のライフサイクル（作成、割り当て、クリーンアップ）を管理する独自実装クラスである。グラフ定義の`environment_mode: "create"`ノード数に基づいて環境プールを準備し、各ノードに一意の環境を割り当てる。プロジェクト言語に応じた適切なDockerイメージを選択して環境を作成する。

#### 8.8.2 責務

1. **環境名マッピング管理**: 環境名（python、node等）からDockerイメージ名へのマッピングを管理
2. **環境プール管理**: ワークフロー開始時に必要な数のDocker環境を選択されたイメージで一括作成
3. **環境割り当て**: 各`environment_mode: "create"`ノードに一意の環境IDを割り当て、`environment_mode: "inherit"`ノードは`execution_environments`から環境IDを取得
4. **環境分離**: 並列実行されるノードが独立した環境で動作することを保証
5. **環境クリーンアップ**: ワークフロー完了時または異常終了時に全環境を削除
...
| メソッド | 説明 |
|---------|------|
| `prepare_environments(count, environment_name, mr_iid, node_ids)` | 指定された環境名のイメージでcount個のDocker環境を一括作成し、環境IDリストを返す。環境IDは`codeagent-{environment_name}-mr{mr_iid}-{node_id}`形式で生成される |
...
#### 8.8.5 環境の引き継ぎ（Environment Inheritance）
...
1. **実行エージェントが環境IDを出力**:
   - 実行エージェント（code_generation、bug_fix等）は、使用したDocker環境のIDをワークフローコンテキストに書き込む
   - コンテキストキー: `execution_environments`（辞書型、単一・並列両方対応）
   - 各エージェントは自身のエージェント定義IDをキーとして辞書に書き込む
   - スコープ: `workflow`（ワークフロー全体で共有）

2. **レビューエージェントが同じ環境IDを使用**:
   - レビューエージェント（code_review等）は、ワークフローコンテキストから環境IDを読み取る
   - 読み取った環境IDを使用して、ExecutionEnvironmentManagerに既存環境の使用を指示
   - これにより、実行エージェントが生成したファイルに直接アクセス可能

3. **MCPToolWrapperの拡張**:
   - `EnvironmentAwareMCPClient`を拡張し、コンテキストから環境IDを取得する機能を追加
   - コンテキストに環境IDが存在する場合は既存環境を使用し、存在しない場合はノードIDから新規環境を取得
   - 辞書型キー（`execution_environments`）を使用し、単一・並列両方のエージェントに対応

**マルチエージェント時の環境引き継ぎ**:

```mermaid
graph TB
    Plan[計画フェーズ] --> Branch[サブブランチ作成]
    Branch --> Fast[code_generation_fast<br/>env_1]
    Branch --> Std[code_generation_standard<br/>env_2]
    Branch --> Cre[code_generation_creative<br/>env_3]
    
    Fast -->|env_id: env_1| Rev[code_review<br/>3つの環境を順次確認]
    Std -->|env_id: env_2| Rev
    Cre -->|env_id: env_3| Rev
    
    Rev -->|selected: env_2| Test[test_execution_evaluation<br/>env_2を使用]
    
    style Fast fill:#99ccff
    style Std fill:#99ccff
    style Cre fill:#99ccff
...
test_execution_evaluationエージェントは、`selected_implementation`から環境IDを取得し、その環境でテストを実行する。これにより、選択された実装のみが後続の処理に引き継がれる。
...
1. **環境準備**: WorkflowFactory が `prepare_environments(count=3, environment_name="python", mr_iid=123, node_ids=["code_generation_fast", "code_generation_standard", "code_generation_creative"])` を呼び出し
   - 生成される環境ID:
     - `codeagent-python-mr123-code_generation_fast`
     - `codeagent-python-mr123-code_generation_standard`
     - `codeagent-python-mr123-code_generation_creative`
2. **環境割り当て**: 各code_generationノード初回実行時に環境プールから環境IDを割り当て
   - `code_generation_fast` → `codeagent-python-mr123-code_generation_fast`を割り当て、execution_environments["code_generation_fast"] = "codeagent-python-mr123-code_generation_fast"
   - `code_generation_standard` → `codeagent-python-mr123-code_generation_standard`を割り当て、execution_environments["code_generation_standard"] = "codeagent-python-mr123-code_generation_standard"
   - `code_generation_creative` → `codeagent-python-mr123-code_generation_creative`を割り当て、execution_environments["code_generation_creative"] = "codeagent-python-mr123-code_generation_creative"
3. **並列実行**: 各ノードは割り当てられた環境で独立して動作
4. **環境引き継ぎ**: test_execution_evaluationは選択されたcode_generationノードの環境IDをexecution_environmentsから読み取って使用（environment_mode="inherit"）
   - 例: code_generation_standardが選択された場合、`codeagent-python-mr123-code_generation_standard`を使用
5. **クリーンアップ**: ワークフロー完了時に3つの環境を一括削除

**注意**: test_execution_evaluationはenvironment_mode="inherit"のため、新規環境は割り当てられず、選択されたcode_generationノードの環境を再利用する。
...
- **ConfigurableAgent（execution役割）**:
  - 操作: タスク状態更新（running → completed）、ワークフローコンテキスト更新（execution_environments書き込み）
...
| ツール名 | 説明 | パラメータ |
|---------|------|----------|
| `create_todo_list` | Todoリスト作成（GitLab自動同期） | project_id, issue_iid/mr_iid, todos |
| `get_todo_list` | Todoリスト取得 | project_id, issue_iid/mr_iid |
| `update_todo_status` | Todo状態更新（GitLab自動同期） | todo_id, status |
| `add_todo` | Todo追加（GitLab自動同期） | project_id, issue_iid/mr_iid, title, description, parent_todo_id |
| `delete_todo` | Todo削除（GitLab自動同期） | todo_id |
| `reorder_todos` | Todo順序変更（GitLab自動同期） | todo_ids |
...
**GitLab自動同期**: 
Todoツール（`create_todo_list`, `update_todo_status`, `add_todo`, `delete_todo`, `reorder_todos`）は、実行後に自動的にGitLab Issue/MRのdescriptionまたはコメントにMarkdown形式でTodoリストを投稿する。これにより、GitLab UI上でも進捗を確認できる。エージェントが明示的に同期ツールを呼ぶ必要はない。
...
### 15.2 Microsoft Agent Framework (Python)標準機能の活用

**参考**: [Agent Framework (Python)](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)

| 標準機能 | 用途 | メリット | ドキュメント |
|---------|------|---------|------------|
| Agent (Executor) | AIエージェント基盤 | LLM呼び出し、ワークフロー実行 | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |
| BaseHistoryProvider | 会話履歴管理 | メッセージ管理、独自Providerで永続化可能 | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |
| OpenTelemetry統合 | 分散トレーシング | 監視、デバッグ、パフォーマンス分析 | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |
| Filters | リクエスト処理 | エラーハンドリング、ログ記録、認証の統一 | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |
| OpenAIChatClient/AzureOpenAIChatClient | LLM統合 | 複数プロバイダー対応、統一API | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |
| FunctionTool / MCPStdioTool | ツール/プラグイン | GitLab API、MCP、カスタムツール統合 | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |
| Process Framework | Workflow/Executor管理 | ワークフロー実行、Executor管理 | [Agent Framework](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework) |

### 15.3 coding_agentからの移植対象ファイル
...
| **`context_storage/message_store.py`** | `MessageStore` | LLM会話履歴保存 | **PostgreSqlChatHistoryProviderで再実装** | **[BaseHistoryProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)パターンを活用** |
| **`context_storage/summary_store.py`** | `SummaryStore` | 要約保存 | **PlanningContextProviderに統合** | **[BaseContextProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)パターンを活用** |
...
**重要**: coding_agentのファイルベースのコンテキスト管理を、PostgreSQLベースの独自コンテキスト管理クラスで再設計する。[BaseHistoryProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)と[BaseContextProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)パターンを参考にすることで、保守性が向上する。

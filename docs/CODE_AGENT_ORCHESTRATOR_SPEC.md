...
- **execution**: プラン取得→環境ID取得（`get_environment_id()`で`node_id`を使用）→LLM呼び出し（実装/生成）→ファイル操作（MCPツール）→git操作→`execution_environments[agent_definition_id] = env_id`として保存→コンテキスト保存
- **review**: MR差分取得→`execution_environments`辞書から環境ID取得→LLM呼び出し（レビュー）→コメント生成→GitLab投稿→コンテキスト保存

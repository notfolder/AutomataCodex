...
|------------|------|---------|---------------------|
| **IssueToMRConverter** | Issue→MR変換 | 独自Workflow実装 | [coding_agent](https://github.com/notfolder/coding_agent)を参考にして、独自のワークフロー管理クラスを実装 |
...
| コンポーネント | 責務 | Agent Frameworkクラス | [coding_agent](https://github.com/notfolder/coding_agent)参照・実装方針 |
|------------|------|---------|---------------------|
| **WorkflowFactory** | ワークフロー生成・環境準備 | 独自実装 | グラフ定義からワークフローを動的に生成し、Docker環境を準備する |
...
- `_setup_environments(graph_def)`: グラフ定義内で`requires_environment: true`のノードに対してDocker環境を準備する（`EnvironmentSetupExecutor`が担当し、ワークフロー実行開始直後に呼び出される）
...
**実装方法**:
[coding_agent](https://github.com/notfolder/coding_agent)を参考にして、ワークフロー管理クラスを実装する。[clients/gitlab_client.py](https://github.com/notfolder/coding_agent/blob/main/clients/gitlab_client.py)はそのまま流用する。
...
詳細な実装方法は[セクション8.9 Middleware機構](#89-middleware機構)を参照。
...
5. `WorkflowBuilder`が`Workflow`オブジェクトを構築
   ...
   - `ExecutorFactory`から必要な`Executor`を生成して追加（UserResolverExecutor、`EnvironmentSetupExecutor`等）
...
6. `ExecutorFactory`が各`Executor`を生成
   ...
   - `EnvironmentSetupExecutor`: グラフ定義で`requires_environment: true`のノード数分のDocker環境を構築（ワークフロー実行開始直後に呼び出され、全実行系ノードが使用できるよう事前準備する）

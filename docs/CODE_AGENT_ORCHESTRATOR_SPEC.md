...
各エージェントノードの実行中、以下の3つの固定イベントでMRにコメントを投稿する。ラベルとなるノード名は`agent_definition_id`を使用するため、グラフ定義を変更して新規ノードを追加した場合でも固定の記述変更なしに対応できる。

1. **start（ノード開始）**
   - メッセージ: "⏳ [{agent_definition_id}] 処理を開始します"
   - 内容: ノードID、開始時刻

2. **llm_response（LLM応答取得）**
   - メッセージ: "🤖 [{agent_definition_id}] LLM応答を取得しました"
   - 内容: 応答の要約（details.summary）。内容が長い場合は`<details>`で折りたたむ

3. **complete（ノード完了）**
   - メッセージ: "✅ [{agent_definition_id}] 完了しました"
   - 内容: 実行時間、変更ファイル数などdetailsに含まれる情報

上記に加え、ワークフロー全体の開始・完了・エラー時には以下を投稿する：

4. **タスク全体開始時**
   - メッセージ: "🚀 タスク処理を開始します"
   - 内容: タスク種別、使用ワークフロー定義名、開始時刻

5. **エラー発生時**
   - メッセージ: "❌ [{agent_definition_id}] エラーが発生しました"
   - 内容: エラー種別、エラーメッセージ、リトライ情報
...
各イベントのコメントは以下の形式で生成される。`{agent_definition_id}`の部分に実際の`agent_definition_id`値（例: `code_generation_fast`、`security_validation`等）が入るため、グラフ定義を変更しても表示ラベルが自動的に変わる。

```
ノード開始:
  ⏳ [{agent_definition_id}] 処理を開始します
  - ノードID: {node_id}
  - 開始時刻: {timestamp}

LLM応答取得:
  🤖 [{agent_definition_id}] LLM応答を取得しました
  {details.summary}
  （内容が長い場合は<details>で折りたたむ）

ノード完了:
  ✅ [{agent_definition_id}] 完了しました
  - 実行時間: {elapsed}
  {detailsに含まれる情報}

エラー:
  ❌ [{agent_definition_id}] エラーが発生しました
  - エラー: {error_type}: {error_message}
  - リトライ: {retry_count}/{max_retry}回目

タスク全体完了:
  ✨ タスク完了
  - 実行時間: {total_elapsed}
  - 変更ファイル: {file_count}ファイル
  次のアクション: MRのレビューをお願いします。
```

**並列実行時の表示**

`multi_codegen_mr_processing`のように複数ノードが並列実行される場合、各ノードがまず個別にstart/llm_response/completeイベントを投稿する。`agent_definition_id`が各ノードで異なる（例: `code_generation_fast`、`code_generation_standard`）ため、コメントの先頭ラベルで自動的に区別できる。
...
- イベント種別（start / llm_response / complete / error）と`agent_definition_id`を組み合わせてコメントフォーマットを生成する。`role`には依存しない

**主要メソッド**:
- `report_progress(mr_iid, event, agent_definition_id, node_id, details)`: 指定イベントの進捗コメントをMRに投稿する
- `format_progress_comment(event, agent_definition_id, node_id, details)`: イベントと`agent_definition_id`からMarkdown形式のコメントを生成する
- `add_progress_log(mr_iid, event, agent_definition_id, node_id, details)`: 進捗情報をコンテキストストレージに記録する
...
**参考**: [Agent Framework (Python)](https://github.com/microsoft/agent-framework/tree/main/python/packages/core)

| 標準機能 | 用途 | メリット | ドキュメント |
|---------|------|---------|------------|
| Agent (Executor) | AIエージェント基盤 | LLM呼び出し、ワークフロー実行 | [Agent](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_agents.py) / [Executor](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_workflows/_executor.py) |
| BaseHistoryProvider | 会話履歴管理 | メッセージ管理、独自Providerで永続化可能 | [BaseHistoryProvider](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_sessions.py) |
| OpenTelemetry統合 | 分散トレーシング | 監視、デバッグ、パフォーマンス分析 | [observability.py](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/observability.py) |
| Filters | リクエスト処理 | エラーハンドリング、ログ記録、認証の統一 | [_middleware.py](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_middleware.py) |
| OpenAIChatClient/AzureOpenAIChatClient | LLM統合 | 複数プロバイダー対応、統一API | [OpenAIChatClient](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/openai/_chat_client.py) / [AzureOpenAIChatClient](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/azure/_chat_client.py) |
| FunctionTool / MCPStdioTool | ツール/プラグイン | GitLab API、MCP、カスタムツール統合 | [FunctionTool](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_tools.py) / [MCPStdioTool](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_mcp.py) |
| Process Framework | Workflow/Executor管理 | ワークフロー実行、Executor管理 | [_workflows/](https://github.com/microsoft/agent-framework/tree/main/python/packages/core/agent_framework/_workflows) |
...
| **`context_storage/message_store.py`** | `MessageStore` | LLM会話履歴保存 | **PostgreSqlChatHistoryProviderで再実装** | **[BaseHistoryProvider](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_sessions.py)パターンを活用** |
| **`context_storage/summary_store.py`** | `SummaryStore` | 要約保存 | **PlanningContextProviderに統合** | **[BaseContextProvider](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_sessions.py)パターンを活用** |
...
**重要**: coding_agentのファイルベースのコンテキスト管理を、PostgreSQLベースの独自コンテキスト管理クラスで再設計する。[BaseHistoryProvider](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_sessions.py)と[BaseContextProvider](https://github.com/microsoft/agent-framework/blob/main/python/packages/core/agent_framework/_sessions.py)パターンを参考にすることで、保守性が向上する。

...
- `get_environment_id()`: `env_ref`の値に応じて環境IDを返す。"plan"の場合はコンテキストの`plan_environment_id`を返す。"1"/"2"/"3"の場合はコンテキストの`branch_envs[N]`を返す。省略の場合はNoneを返す。環境自体は同一分岐内のExecEnvSetupExecutorが事前作成済みみ
...
- **execution**: プラン取得→`get_environment_id()`で`branch_envs[1]`から環境ID取得→進捗報告（開始）→LLM呼び出し（実装/生成）→進捗報告（LLM応答）→ファイル操作（MCPツール）→git操作→進捗報告（実行完了）→コンテキスト保存
...
6. `branch_envs: {1: env_id_1, 2: env_id_2, ...}`をワークフローコンテキストに保存
...
2. **実行環境の分岐別作成**: 各タスク分岐内の`exec_env_setup_*`ノード（ExecEnvSetupExecutor）がplanning完了後に実行され、ノード設定の`env_count`数分の実行環境を作成する。作成された環境IDは`branch_envs: {1: env_id, 2: env_id, ...}`としてコンテキストに保存される
...
| `branch_envs` | dict | 番号をキーとした環境ID辞書（`{1: env_id, 2: env_id, ...}`） | ExecEnvSetupExecutor実行時 |

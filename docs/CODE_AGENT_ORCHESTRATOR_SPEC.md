...
|------------|------|---------|---------------------|
| **Producer** | Issue/MR検出・キューイング | Python + GitLab API + RabbitMQ | `producer.py`（新規）<br/>`queueing.py` |
| **Consumer** | タスクデキュー・処理振り分け | Python + RabbitMQ | `consumer.py`（新規）<br/>`queueing.py` |
| **TaskHandler** | タスク処理分岐（Issue/MR判定） | Python | `handlers/task_handler.py` |
| **RabbitMQ** | 分散タスクキュー | RabbitMQ（durable queue） | - |
| **TaskGetterFromGitLab** | GitLab API経由タスク取得 | Python + GitLab API | `producer.py: TaskGetterFromGitLab`（新規） |

#### 2.3.2 Issue→MR変換 Layer（前処理フロー）

| コンポーネント | 責務 | 実装技術 | coding_agent流用ファイル |
|------------|------|---------|---------------------|
| **IssueToMRConverter** | Issue→MR変換 | Python + GitLab API + LLM | `handlers/issue_to_mr_converter.py`（新規） |
| **Branch Naming Agent** | ブランチ名生成 | LLM | `clients/llm_*.py` 参考 |
...
| **MCPToolClient** | ツール実行 | Python + MCP | `clients/mcp_tool_client.py` |
...
| **LLMClient** | LLM通信 | OpenAI SDK | `clients/llm_*.py` 参考 |
...
| エージェント名 | 役割 | 入力 | 出力 |
|--------------|------|------|------|
| Producer | Issue/MR検出・キューイング | GitLab API | RabbitMQキュー |
| Consumer | タスク処理振り分け | RabbitMQキュー | Issue→MR変換 or MR処理 |
| Task Classifier Agent | タスク分類 | Issue内容 | コード生成/バグ修正/ドキュメント生成/テスト作成 |
| User Resolver Agent | ユーザー情報取得 | メールアドレス | LLM設定 |
| Planning Agent | 実行計画生成 | タスク内容 | アクションプラン |
...
| MR Manager Agent | MR操作 | 実行結果 | MR更新 |
...
2. 計画フェーズ
   ...
   - タスクの分解
   - アクション系列の生成
   - **Todoリストの作成**: タスクをTodoリスト形式で構造化し、`create_todo_list` ツールで永続化
3. 計画のコンテキストストレージへの保存
4. **GitLabへのTodoリスト投稿**: `sync_to_gitlab` ツールでIssue/MRにTodoリストを投稿
...
**LLMプロンプト**: 英語で記述
```
You are a test execution and evaluation agent. Analyze test results and determine the cause of failures.

Instructions:
1. Execute all relevant tests (unit, integration, E2E)
2. Collect test results and coverage information
3. If tests fail, analyze the cause:
   - Implementation issue: Bug in the code, logic error, missing error handling
   - Test issue: Test case error, environment dependency, timeout
4. Provide detailed failure analysis and fix recommendations
...
```mermaid
flowchart TD
    Start([Issue検出]) --> Check{Issue→MR変換<br/>必要?}
    Check -->|No| Skip[スキップ]
    Check -->|Yes| GetIssue[Issue情報収集]
    ...
    CreateMR --> TransferContent{コンテンツ転記<br/>設定あり?}
    TransferContent -->|Yes| Transfer[Issueコメントを<br/>MRに転記]
    TransferContent -->|No| NotifyIssue[元Issueに通知コメント]
    Transfer --> NotifyIssue
    
    NotifyIssue --> SetupAuto{自動タスク化<br/>設定あり?}
    SetupAuto -->|Yes| AddLabel[MRにラベル追加]
    SetupAuto -->|No| UpdateStatus[Issue Done化]
...
#### 5.0.3 IssueToMRConverter（coding_agentからの移植）

以下のコンポーネントをcoding_agentから移植して実装:
...
flowchart TD
    ...
    Complete -->|Yes| Finish[タスク完了<br/>MR Manager Agent]
...
**LLMプロンプト**: 英語で記述
```
You are a task classifier. Analyze the issue/MR and determine the task type.

Instructions:
1. Read the issue title, description, and labels
2. Identify task type: bug_fix, documentation, test_creation, code_review, or doc_review
3. List related files in the repository
4. Identify dependencies

Available tools: list_repository_files, read_file, search_code
...
**目的**: コード生成系タスクで仕様ファイルが存在しない場合、仕様書を自動生成して終了する

**使用エージェント**: Planning Agent → Documentation Agent → Documentation Review Agent

**実行条件**: 
- タスク種別が `code_generation`, `bug_fix`, または `test_creation`
- かつ、関連する仕様/設計ファイルが存在しない

**実行内容**:
1. **Planning Agentによるドキュメント生成計画**
   - 仕様書作成のための計画を生成
   - Issue/MR内容から必要な仕様項目を抽出
   - Todoリストを作成
2. **Documentation Agentによる仕様作成**
   - Issue/MR内容から仕様を抽出
   - 既存コードの分析
   - Markdown形式で仕様/設計書を作成
   - 図表（Mermaid）の追加
3. **Documentation Review Agentによる自動レビュー**
   - 内容の正確性確認
   - 完全性チェック
   - 構造の妥当性評価
   - レビュー結果をコンテキストに記録
4. **リフレクション**
   - レビュー結果を評価
   - 重大な問題があれば再計画（ステップ1に戻る）
   - 問題なければ次のステップへ
5. **タスク完了**
   - 作成した仕様書のパスを記録
   - GitLabにコミット、MRコメントで仕様書作成完了を通知
   - タスク終了（コード生成/バグ修正/テスト作成は実行しない）
   
**注意**: 仕様書作成後、コード実装を行う場合は新たなIssue/MRを作成して実行する
...
```python
def report_progress(mr_iid: int, phase: str, message: str, details: dict = None):
    """
    MRに進捗報告コメントを投稿
    
    Args:
        mr_iid: MR IID
        phase: フェーズ名 (start, planning, execution, review, test, complete, error)
        message: メッセージ
        details: 追加詳細情報
    """
    comment_body = format_progress_comment(phase, message, details)
    gitlab_client.add_comment_to_mr(mr_iid, comment_body)
    
    # コンテキストにも記録
    context_manager.add_progress_log(mr_iid, phase, message, details)
```
...
```python
# Planning Agent
report_progress(mr_iid, "planning", "📋 実行計画を生成しました", {
    "steps": ["step1", "step2", "step3"],
    "estimated_time": "15 minutes"
})

# Code Generation Agent
report_progress(mr_iid, "execution", "⏳ コード生成中...", {
    "progress": "1/3",
    "current_task": "ユーザー認証機能の実装"
})

# Test Execution Agent
report_progress(mr_iid, "test", "✅ テスト実行結果", {
    "success_rate": "100%",
    "coverage": "87%",
    "duration": "12.5s"
})
...
# Error Handler
report_progress(mr_iid, "error", "❌ エラー発生", {
    "error_type": "GitLabAPIError",
...
- **Personal Access Token**: 環境変数 `GITLAB_PERSONAL_ACCESS_TOKEN`
- **スコープ**: api, read_api, read_repository, write_repository
- **ヘッダー**: `PRIVATE-TOKEN: <token>`
...
#### MRマージ
```http
PUT /api/v4/projects/:id/merge_requests/:mr_iid/merge
Content-Type: application/json
...
### 8.2 コンテキストストレージ（追加の永続化機能）

Agent Frameworkの標準状態管理を補完し、タスクの全情報を永続化する。
`https://github.com/notfolder/coding_agent` の `context_storage/*` を流用。
...
```jsonl
{"seq": 1, "role": "system", "content": "...", "timestamp": "...", "tokens": 150}
{"seq": 2, "role": "user", "content": "...", "timestamp": "...", "tokens": 200}
{"seq": 3, "role": "assistant", "content": "...", "timestamp": "...", "tokens": 300}
```

#### current.jsonl (OpenAI API互換)
...
```jsonl
{"seq": 1, "tool_name": "edit_file", "arguments": {...}, "result": "...", "error": null, "timestamp": "..."}
```

#### planning/history.jsonl

```jsonl
{"id": 1, "phase": "planning", "plan": {...}, "timestamp": "..."}
{"id": 2, "phase": "execution", "action_id": "action_1", "result": "...", "timestamp": "..."}
{"id": 3, "phase": "reflection", "reflection": {...}, "timestamp": "..."}
```
...
Agent Frameworkの標準状態管理とは別に、ツールで読んだファイル情報やコマンド実行結果を外部ファイルに出力する。
...
mcp_servers:
  ...
  - name: "todo-management"
    command: ["python", "mcp/todo_management.py"]
    env:
...
**重要**: LLMは直接GitLab APIやGitLab MCP Toolsを呼び出しません。LLMの応答（コード、レビューコメント等）はシステム側が処理し、GitLab API経由でIssue/MRに反映します。

#### GitLab API操作（システム側で実行）

以下の操作はシステム（Producer/Consumer/Agent Framework）がGitLab APIを直接呼び出して実行します：

| 操作 | 説明 | 利用箇所 |
|------|------|----------|
| `create_merge_request` | MR作成 | Issue→MR変換 |
| `add_comment_to_issue` | Issueコメント追加 | Issue→MR変換完了通知 |
| `add_comment_to_mr` | MRコメント追加 | LLM応答の投稿、進捗報告 |
| `update_issue_labels` | Issueラベル更新 | 処理中/完了ラベル付与 |
| `update_mr_labels` | MRラベル更新 | 処理中/完了ラベル付与 |
| `get_file_content` | ファイル内容取得 | コード解析 |
| `create_branch` | ブランチ作成 | Issue→MR変換 |
| `create_commit` | コミット作成 | コード変更のMRへの反映 |
...
- **保管場所**: 環境変数
- **スコープ**: 最小権限（api, read_api, read_repository, write_repository）
- **ローテーション**: 90日ごとに推奨
...
def generate_api_token(user_id: int, secret_key: str) -> str:
    """API認証トークン生成"""
    payload = {
        "user_id": user_id,
...
### 11.4 セキュリティベストプラクティス

1. **環境変数管理**
   - `.env` ファイルはGit管理外
   - 本番環境はシークレット管理サービス使用（AWS Secrets Manager等）

2. **ログ記録**
   - 機密情報（APIキー、パスワード）はログに出力しない
   - マスキング処理を実装

3. **入力検証**
   - ユーザー入力は必ずバリデーション
   - SQLインジェクション対策（ORM使用）
   - コマンドインジェクション対策（パラメータ化）
...
def cleanup_old_tasks(days: int = 30):
    ...
    # ファイルシステムから削除
    for task_dir in Path("contexts/completed").iterdir():
        metadata = json.loads((task_dir / "metadata.json").read_text())
        if datetime.fromisoformat(metadata["completed_at"]) < cutoff_date:

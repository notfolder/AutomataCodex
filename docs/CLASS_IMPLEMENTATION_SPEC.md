...
ExecutionEnvironmentManagerはDocker環境のライフサイクル（作成、割り当て、クリーンアップ）を管理する。
...
#### prepare_environments(count: int) → List[str]
...
2. **環境作成ループ**
   ...
   - 各反復で:
     - 一意の環境ID生成（UUID）
     - docker_client.containers.create()でDockerコンテナ作成

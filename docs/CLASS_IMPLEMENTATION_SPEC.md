...
##### save_token_usage(user_email: str, task_uuid: str, node_id: str, model: str, prompt_tokens: int, completion_tokens: int, total_tokens: int) → None

**処理フロー**:

1. **TokenUsageRecordの構築**
   - 引数をtoken_usage_repositoryへの呼び出し引数として整理する

2. **リポジトリへの保存**
   - token_usage_repository.record_token_usage(user_email, task_uuid, node_id, model, prompt_tokens, completion_tokens)を呼び出してtoken_usageテーブルに記録する
...
1. **タスクのステータス更新**
   - task_repository.update_task_status(task_uuid, 'failed', error_message=error_message)を呼び出し、タスクをfailed状態にしてerror_messageを保存する

2. **エラー詳細のメタデータ保存**
   - error_metadata = {"error": {"node_id": node_id, "category": error_category, "message": error_message, "stack_trace": stack_trace}}を構築する
   - task_repository.update_task_metadata(task_uuid, error_metadata)を呼び出してエラー詳細をタスクメタデータに記録する
...
5. **データベース記録**
   ...
   - user_email、task_uuid、node_id、model、prompt_tokens、completion_tokens、total_tokensを渡す
   - token_usageテーブルに保存

6. **メトリクス送信**
   - metrics_collector.send_metric()を呼び出し
   - metric_name='token_usage_total'
   - labels={'model': model, 'node_id': node_id, 'user_email': user_email}
   - value=total_tokens

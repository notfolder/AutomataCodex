...
Agent Frameworkの[Executor](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を継承する。
...
- **config: AgentNodeConfig** - エージェント定義から取得した設定
  ...
  - prompt_id: プロンプト定義ID
- **agent: Agent** - Agent Frameworkの[Agent](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)インスタンス（LLM呼び出し）
- **tools: list[MCPStdioTool | FunctionTool]** - エージェントが使用するツールリスト（[MCPStdioTool](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)等）
...
5. **Agent Frameworkの[Agent.run()](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)呼び出し**
   - await agent.run([Message(role="user", text=prompt)], session=session)を呼び出し
   - sessionはエージェント生成時に作成したAgentSessionを使用
   - 会話履歴は[BaseHistoryProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)経由で自動的にロードされる（PostgreSqlChatHistoryProvider経由）

6. **LLM応答取得**
   - Agent Frameworkから[Message](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を取得
...
8. **ツール呼び出し処理**
   - Agent Frameworkが[MCPStdioTool](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を自動的に呼び出すため、明示的な実装は不要
...
1. [BaseHistoryProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)（PostgreSqlChatHistoryProvider）からget_messages()でメッセージを取得
2. Agent Frameworkの[Message](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)一覧として返す
...
2. [MCPStdioTool](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)経由でtool_nameのツールを呼び出す（Agent Frameworkが自動的に実行するため、エージェントがツール呼び出しを求めた場合に使用）
3. 結果を辞書形式で返す
...
2. **ツールリスト構築**
   ...
   - 各ツール名について:
     - MCPツールの場合: mcp_client_factory.create_mcp_tool(tool_name, env_id)で[MCPStdioTool](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)オブジェクトを生成してツールリストに追加する
...
4. **ChatClient生成**
   - Agent Frameworkの[OpenAIChatClient](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)または[AzureOpenAIChatClient](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を生成
...
MCPClientFactoryはMCPサーバーへの[MCPStdioTool](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を生成し、Agentのtoolsリストに渡せる形で返す。Agent FrameworkはMCPStdioToolをAgentのコンストラクタに直接渡す設計のため、Kernelへの登録は不要。
...
##### create_mcp_tool(server_name: str, env_id: str) → [MCPStdioTool](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)
...
6. **環境IDをコンテキストに保存**
   - ctx.set_state('environment_ids', environment_ids)
...
Agent Frameworkの[BaseHistoryProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を継承する。
...
##### async def get_messages(self, session_id: str, **kwargs) -> list[[Message](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)]
...
3. **Message変換**
   - 各行についてAgent Frameworkの[Message](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)オブジェクトを生成
...
Agent Frameworkの[BaseContextProvider](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework)を継承する。
...
##### async def before_run(self, *, agent, session, context: [SessionContext](https://github.com/microsoft/agent-framework/tree/main/python/packages/agent_framework), state: dict) -> None
...
## 10. GuidelineLearningAgent（学習エージェント）
...
- 他のエージェント（ConfigurableAgent）と異なり、`gitlab_client`と`git_client`を例外的に保持する固定実装
...
4. **ガイドライン読み込み**
   - `git_client.read_file("PROJECT_GUIDELINES.md")`でファイルを読み込む
...
6. **ガイドライン更新**（`should_update=true`の場合のみ）
   - `git_client.write_file("PROJECT_GUIDELINES.md", response.updated_guidelines)`でファイルを書き込む
   - `git_client.commit_and_push(message="自動学習: ガイドライン更新", files=["PROJECT_GUIDELINES.md"])`でコミット＆プッシュ
...
GuidelineLearningAgentは唯一`git_client`を保持してcommit & pushを実行できる。これはシステムの明示的な例外として設計されている。

**例外が許容される条件**:
- 本エージェントは全ワークフロー完了後に実行される（他のgit操作との競合なし）
- 対象ファイル（PROJECT_GUIDELINES.md）は通常の実装ファイルと独立している（影響範囲限定）
- `WorkflowFactory._inject_learning_node()`でのみインスタンス生成され、`gitlab_client`・`git_client`はここでのみ注入される

**制約**:
- 他のエージェント（ConfigurableAgent等）はgit操作クライアントを保持しない
- GitLabへのpush先はタスクのブランチに限定される

...
2. BranchMergeExecutorインスタンスを生成し、gitlab_client・git_clientを渡す
3. 返却
...
- **gitlab_client: GitLabClient** - GitLabクライアント（MR/ブランチ操作用）
- **git_client: GitClient** - Gitクライアント（マージ操作用）
...
4. **ブランチマージ**
   - git_clientを使用して選択ブランチをoriginal_branchにマージ（fast-forward優先、コンフリクト発生時はエラーを記録して処理を中断）

...
7. **ConfigurableAgentインスタンス生成**
   ...
   - `env_ref`が"1"/"2"/"3"の場合、environment_idにはワークフローコンテキストの`branch_envs[N]`をビルド時に設定する
   - `env_ref`が省略の場合、environment_idにはNoneを設定する
...
5. **branch_envsをコンテキストに保存**
   - `{1: env_id_1, 2: env_id_2, ...}`形式の辞書を生成して`ctx.set_state('branch_envs', branch_envs)`を呼び出し
...
- Executor群: BaseExecutor、UserResolverExecutor、ContentTransferExecutor、PlanEnvSetupExecutor、ExecEnvSetupExecutor

...
1. **タスクMR/Issue IID取得**
   - context.get_state("task_mr_iid", scope_name='workflow')でMR IIDを取得
   - 存在しない場合は"task_issue_iid"を取得
   - task_iid変数に保存

2. **入力データ取得**
   - config.input_keysをループ
   - 各キーについてcontext.get_state(key, scope_name='workflow')を呼び出し
...
11. **出力データ保存**
   ...
   - 各キーについてcontext.set_state(key, value, scope_name='workflow')を呼び出し
   - LLM応答から抽出した値を保存
...
1. keysをループ
2. 各キーについてcontext.get_state(key, scope_name='workflow')を呼び出し
3. 取得した値を辞書に格納して返す
...
3. context.set_state(key, value, scope_name='workflow')を呼び出し
...
1. context.get_state(key, scope_name)を呼び出し
2. 値を返す
...
1. context.set_state(key, value, scope_name)を呼び出し
...
1. **タスク情報取得**
   - context.get_state('task_identifier', 'workflow')を呼び出し
...
4. **ワークフローコンテキストに保存**
   - context.set_state('user_email', user_email, 'workflow')
   - context.set_state('user_config', user_config, 'workflow')
...
1. **Issue情報取得**
   - context.get_state('issue_iid', 'workflow')を呼び出し
   - context.get_state('project_id', 'workflow')を呼び出し
...
3. **MR情報取得**
   - context.get_state('mr_iid', 'workflow')を呼び出し
...
5. **転記数記録**
   - context.set_state('transferred_comments_count', count, 'workflow')
...
1. **MR IID取得**
   - context.get_state('task_mr_iid', 'workflow')を呼び出し
...
6. **環境IDをコンテキストに保存**
   - context.set_state('environment_ids', environment_ids, 'workflow')
...
7. **新規コメントがある場合**
   - context.set_state('user_new_comments', new_comments, 'workflow')
   - MiddlewareSignalを生成:
     - action='redirect'
...
4. **リトライ判定**
   ...
   - context.get_state('retry_count', 'workflow')を呼び出し
   - retry_countがmax_attemptsより小さい場合: リトライ実行

5. **リトライ実行**
   - retry_count += 1
   - context.set_state('retry_count', retry_count, 'workflow')
...
6. **リトライ上限到達またはリトライ不可の場合**
   ...
   - タスク状態更新: context.set_state('status', 'failed', 'workflow')
   - MiddlewareSignalを生成:
     - action='abort'
...
```
ChatCompletionAgent (Agent Framework標準クラス)
